class Craftsman
  def Craftsman.configure(config, settings)
    # Set The VM Provider
    ...
    # Configure The Box
    config.vm.box = settings['box'] ||= 'angusfretwell/journeyman'
    config.vm.box_version = settings['version'] ||= '>= 0.1.0'
    config.vm.hostname = settings['hostname'] ||= 'craftsman'
    ...
    config.vm.provider 'virtualbox' do |vb|
      vb.name = settings['name'] ||= 'craftsman'
    ...
    if settings.has_key?('variables')
      settings['variables'].each do |var|
        ...
        config.vm.provision 'shell' do |s|
            s.inline = 'echo \'\n# Set Craftsman Environment Variable\nexport $1=$2\' >> /home/vagrant/.profile'
            s.args = [var['key'], var['value']]
        end
      end
